...
class EGNN(nn.Module):
    ...
    def forward(self, h, x, node_mask):
        # Edit Emiel: Remove velocity as input

        bs, n_nodes = node_mask.shape

        edges = self.get_adj_matrix(n_nodes, bs, self.device)
        edge_index = [edge.to(x.device) for edge in edges]
        ...
        # x_init = x.view(bs*n_nodes, -1).clone()
        ...
        return h.view(bs, n_nodes, -1), x.view(bs, n_nodes, -1)
